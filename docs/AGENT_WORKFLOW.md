...
- **物流延迟 vs 包裹丢失拆分**：延迟表达（"一个月没到"、"hasn't arrived"）走 `logistics_policy`，不走 `package`；损坏/丢失表达（"破损"、"丢失"、"damaged"）仍走 `package`
- **英文物流延迟识别**：新增 "hasn't arrived"、"taking too long"、"shipping delay" 等关键词
- **退款 vs 物流消歧**：当 "退款多久能到账" 同时匹配退款和物流关键词时，退款优先
- **多意图回答优化**：订单取消 + 退款查询的回答同时覆盖两个方面
- **引用多样性**：citation 优先来自不同 doc_id，避免全是同一文档的多个 chunk
