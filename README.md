...
线上 smoke 已通过（M7）。Render 免费实例可能冷启动，首次访问需等待 30–90 秒。
...
- 安全选择：前端只传 `llm_profile`，后端白名单限制 mock / deepseek / doubao / mimo，并由后端环境变量解析真实配置。
- 前端展示：React + Vite + TypeScript + Tailwind 搭建客服 Agent Console，展示问答、模型 profile、fallback 和评估相关 metadata。
- 部署演示：后端部署到 Render，前端部署到 Vercel，线上 smoke 已通过（M7）。

### 结果

- 工程质量：后端测试 passed，Ruff All checks passed，frontend build passed。
- 检索结果：Retrieval Eval 20 cases，Recall@5 90%，MRR 0.785。Recall@5 表示 top-5 检索结果命中预期文档的比例，MRR 用于观察相关文档是否排在更靠前的位置。
- 回答结果：Answer Eval 122 cases，citation hit rate 95.90%，pass rate 60.66%。v1.3.0 优化后 citation hit rate 提升 12.29 个百分点，pass rate 提升 13.94 个百分点，fallback rate 从 13.11% 降至 0.82%。
- Bad Case Bank：131 条典型客服 Bad Case，覆盖 customs/refund/logistics/payment/order/package/return/exchange/address/coupon/out_of_scope 共 11 个场景。bad_case_eval 结构性通过率 97.71%（128/131），citation 覆盖率 97.54%。
- 安全与可演示性：本地 smoke 中 mock / deepseek / doubao / mimo fallback 均通过；无真实模型 key 也能演示完整链路。
- Real Mimo 验证：已通过 Render 后端环境变量验证 Mimo 真实 LLM profile；真实 key 仅保存在 Render，前端只传 llm_profile，不接触任何密钥。answer_source=real_llm，llm_model=mimo-v2.5-pro，回答比 Mock 更自然。
- 访问入口：Render + Vercel 部署预览，线上 smoke 已通过。
...
- bad case eval：131 cases，pass rate 97.71%，citation coverage 97.54%，覆盖 11 个场景
...
- answer pass rate：基于当前 answer eval 规则得到的回答通过率。
- fallback rate：触发兜底回复的比例。

优化记录：

- v1.3.0 优化前（baseline）：pass rate 46.72%，citation hit rate 83.61%，fallback rate 13.11%
- v1.3.0 优化后：pass rate 60.66%（+13.94pp），citation hit rate 95.90%（+12.29pp），fallback rate 0.82%（-12.29pp）
- 详细报告：docs/RAG_QUALITY_IMPROVEMENT_REPORT.md
...
- Frontend：M7 完成，RAG 展示增强。
- Release tag：v1.2.0-demo（已保留），v1.3.0-quality（待验证）。

Roadmap（已完成）：

- M0–M6：RAG + Eval + Agent Workflow + API + LLM Adapter。
- Frontend M1–M6.5：React scaffold → 单栏聊天 → API 接入 → 响应式 → 长回答折叠。
- Frontend M7：线上 smoke、最终文档、release checklist、tag。
