...
4. **语气要求**：礼貌、简洁、专业、安抚。先安抚客户，再说明情况，最后提供解决方案或建议。
5. **如果无法确定**：回复"抱歉，我无法确认这个问题的答案，建议您联系人工客服。"
6. **只回答当前问题**。不要罗列所有检索到的信息，只选择与用户问题最相关的内容回答。
7. **回答控制在 4-8 行**，列点不超过 5 条。

## 回答格式
请基于上述证据，用客服语气回答用户问题。直接回答，不要在回答中提及文档编号、证据编号或内部系统信息。
"""
    return prompt
...
## 回答格式
请基于物流查询结果，用客服语气回答用户问题。直接回答，不要提及内部系统信息。
"""
    return prompt
